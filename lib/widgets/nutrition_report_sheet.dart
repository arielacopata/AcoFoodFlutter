...
class _NutritionReportSheetState extends State<NutritionReportSheet> {
  ...
  static const List<String> page4Nutrients = [
    'alanine',
    'arginine',
    'asparticAcid',
    'glutamicAcid',
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: const Text('Reporte completo con gráficos (4 páginas)'),
              onTap: () {
                Navigator.pop(bottomSheetContext);
  ...
  Future<void> _exportPdfReport(BuildContext context) async {
    ...
    // PÁGINA 4: AMINOÁCIDOS NO ESENCIALES
    pdf.addPage(
      pw.Page(
        pageFormat: PdfPageFormat.a4,
        margin: const pw.EdgeInsets.all(32),
        build: (pw.Context context) {
          return pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              _buildPdfHeader(dateStr, '4/4'),
              pw.SizedBox(height: 20),
              _buildSectionTitle('AMINOÁCIDOS NO ESENCIALES'),
              pw.SizedBox(height: 8),
              ...
              pw.Container(
                ...
                child: pw.Text(
                  'Incluye aminoácidos no esenciales y condicionalmente esenciales. '
                  'Si bien el cuerpo puede generarlos, bajo ciertas condiciones '
                  '(estrés, enfermedad, crecimiento, ejercicio intenso) puede '
                  'beneficiarse de un aporte externo.',
                  style: const pw.TextStyle(
                    fontSize: 10,
                    color: PdfColors.grey700,
                  ),
              ...
              // Usar el método helper para cada aminoácido
              _buildNonEssentialAminoAcid(
                'Alanina',
                _getNutrientValue('alanine'),
                'Conversión de glucosa en energía (ciclo glucosa-alanina)',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Arginina',
                _getNutrientValue('arginine'),
                'Precursor del óxido nítrico (vasodilatación, inmunidad)',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Ácido aspártico',
                _getNutrientValue('asparticAcid'),
                'Participa en la síntesis de ADN/RNA y ciclo de Krebs',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Ácido glutámico',
                _getNutrientValue('glutamicAcid'),
                'Neurotransmisor excitador y precursor del glutamato y GABA',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Glicina',
                _getNutrientValue('glycine'),
                'Formación de colágeno, antioxidante, sueño y metabolismo',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Prolina',
                _getNutrientValue('proline'),
                'Reparación de tejidos, formación de colágeno',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Serina',
                _getNutrientValue('serine'),
                'Síntesis de purinas/pirimidinas, metabolismo de grasas',
              ),
              pw.SizedBox(height: 10),

              _buildNonEssentialAminoAcid(
                'Tirosina',
                _getNutrientValue('tyrosine'),
                'Precursor de dopamina, adrenalina, noradrenalina y hormonas tiroideas',
              ),
              pw.SizedBox(height: 10),
              ...
              _buildNonEssentialAminoAcid(
                'Cisteína',
                _getNutrientValue('cysteine'),
                'Antioxidante, síntesis de proteínas, salud del cabello y piel',
              ),
              pw.SizedBox(height: 10),
              ...
              _buildNonEssentialAminoAcid(
                'Glutamina',
                _getNutrientValue('glutamine'),
                'Combustible para células inmunes e intestinales, recuperación muscular',
              ),
              pw.SizedBox(height: 10),
              ...
              _buildNonEssentialAminoAcid(
                'Asparagina',
                _getNutrientValue('asparagine'),
                'Síntesis de proteínas, función del sistema nervioso',
              ),
            ],
          );
        },
      ...
      pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Column(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              _buildPdfHeader(dateStr, '5/5'),
              pw.SizedBox(height: 20),

              pw.Text(
                'ÁCIDOS GRASOS SIN CONSENSO',
                style: pw.TextStyle(
                  fontSize: 16,
                  fontWeight: pw.FontWeight.bold,
              ...
              _buildNonEssentialAminoAcid(
                ...
                _getNutrientValue('omega9'),
                'Grasa monoinsaturada esencial para la salud cardiovascular. '
                    'Contribuye a reducir el colesterol LDL ("malo") y aumentar el HDL ("bueno"), '
                    'además de modular la inflamación y mejorar la sensibilidad a la insulina. '
                    'Un aporte externo puede ser beneficioso incluso aunque el cuerpo pueda sintetizarlo. '
                    'Las recomendaciones varían: 10-20 % de las calorías totales '
                    '(para este día: ${(_getTotalCalories() * 0.10 / 9).toStringAsFixed(0)}-${(_getTotalCalories() * 0.20 / 9).toStringAsFixed(0)} g), '
                    'o un rango fijo de 25-45 g diarios según distintas fuentes.',
              ),
            ],
          );
        },
  ...
  double _getNutrientValueFromReport(NutritionReport report, String key) {
    switch (key) {
      ...
      case 'alanine':
        return report.alanine;
      case 'arginine':
        return report.arginine;
      case 'asparticAcid':
        return report.asparticAcid;
      case 'glutamicAcid':
        return report.glutamicAcid;
      case 'glycine':
        return report.glycine;
      case 'proline':
        return report.proline;
      case 'serine':
        return report.serine;
      case 'tyrosine':
        return report.tyrosine;
      case 'cysteine':
        return report.cysteine;
      case 'glutamine':
        return report.glutamine;
      case 'asparagine':
        return report.asparagine;
      default:
        return 0.0;
    }
  }
  ...
  pw.Widget _buildNonEssentialAminoAcid(
    String name,
    double value,
    String function,
  ) {
    return pw.Container(
      ...
      decoration: pw.BoxDecoration(
        border: pw.Border(
          left: pw.BorderSide(color: PdfColors.green, width: 4),
        ),
  ...
  double _getTotalCalories() {
    return _getNutrientValue('calories');
  }
}
